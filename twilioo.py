...
def llm_filter_events(query: str, events: List[Dict], date_context: Dict) -> List[Dict]:
    """Use LLM to filter and format recommendations - ULTRA STRICT VERSION"""
    if not events:
        return []
...
""")
        ...
        for genre, variants in strict_genres.items():
            if any(variant in query_lower for variant in variants):
                requested_genre = genre
                break
        
        prompt = f"""
# TASK: Filter events STRICTLY by user request.
...
## ⚠️ CRITICAL GENRE FILTERING RULES:
"""

        if requested_genre:
            genre_variants = strict_genres[requested_genre]
            prompt += f"""
**USER ASKED FOR: {requested_genre.upper()} MUSIC**

🚨 ULTRA-STRICT RULE: 
- ONLY return events where Music Type contains: {', '.join(genre_variants)}
- If an event's Music Type is "{requested_genre}" or any variant ({', '.join(genre_variants)}), include it
- If an event's Music Type is ANYTHING ELSE (salsa, techno, jazz, rock, etc.), EXCLUDE IT COMPLETELY
- If you find ZERO {requested_genre} events, return {{"recommendations": []}} (empty array)
- DO NOT return mixed genres. DO NOT return "close enough" genres.

Examples:
- User asks "house events" + Event has "Techno" → ❌ EXCLUDE
- User asks "house events" + Event has "Deep House" → ✅ INCLUDE  
- User asks "house events" + Event has "Salsa" → ❌ EXCLUDE
- User asks "tango events" + Event has "Milonga" → ✅ INCLUDE
- User asks "tango events" + Event has "Salsa" → ❌ EXCLUDE
"""
        else:
            prompt += """
**NO SPECIFIC GENRE REQUESTED**
- Return events that match the general vibe/mood
- Prioritize variety
"""
...
If NO events match the strict criteria, return: {"recommendations": []}
"""

        response = openai.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a STRICT event filter. When user asks for a specific music genre, ONLY return that genre. Return empty array if no matches found."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.0,
            max_tokens=2000,
        ...
        # ✅ VERIFY GENRE MATCH (Double-check LLM didn't cheat)
        if requested_genre:
            genre_variants = strict_genres[requested_genre]
            verified_recommendations = []
            
            for rec in recommendations:
                music_type = (rec.get('music_type') or '').lower()
                
                # Check if music_type contains ANY of the requested genre variants
                if any(variant in music_type for variant in genre_variants):
                    verified_recommendations.append(rec)
                else:
                    logger.warning(f"⚠️ LLM returned wrong genre: '{rec.get('music_type')}' when user asked for '{requested_genre}'")
            
            if not verified_recommendations:
                logger.info(f"❌ NO {requested_genre} events found after strict filtering")
                return []
        ...
        logger.info(f"✅ Strict filtering returned {len(validated_recommendations)} events")
        return validated_recommendations[:6]
...
def send_whatsapp_message(to, body, media_url=None):
    ...
    try:
        ...
        if media_url:
            message_data['media_url'] = [media_url]  # ✅ FIXED - Pass as list
...
def ask_chatgpt_expert_fallback(user_input, ai_data, user_language='en'):
    ...
    'hey', 'thanks', 'thank you', 'how are you', 'who are you',
    'what is', 'tell me about', 'explain',
    'whats up', "what's up", 'sup', 'wassup', 'how you doing',
    'hows it going', "how's it going"
    ]
    is_casual = any(pattern.lower() in user_input.lower() for pattern in casual_patterns)
...
# ==============================================================================
# 🚀 PROCESS THREAD - ✅ ONBOARDING FIXED
# ==============================================================================

def process_message_thread(sender, text, message_sid=None):
    """UPDATED: Fixed onboarding to save name/age properly"""
    ...
    try:
        ...
        logger.info(f"📍 Step: {step}, User: {user_name}, Age: {user_age}")
        
        # =================================================================
        # ✅ ONBOARDING FLOW - FIXED VERSION
        # =================================================================
        
        # STEP 1: Welcome → Ask for name/age
        if step == 'welcome':
            send_whatsapp_message(sender, "First, what's your name and age?")
            update_user(conn, sender, {"conversation_step": "ask_name_age", "last_mood": text})
            return
        
        # STEP 2: Collect name/age → Save to DB → Process original query
        if step == 'ask_name_age':
            logger.info(f"📝 Processing name/age: '{text}'")
            
            last_mood = user.get('last_mood')  # Original query
            
            # ===== EXTRACT NAME AND AGE =====
            text_parts = text.strip().split()
            raw_name = text_parts[0] if text_parts else "Friend"
            clean_name = ''.join(filter(str.isalpha, raw_name)) or "Friend"
            age = "".join(filter(str.isdigit, text)) or "25"
            
            logger.info(f"👤 Extracted: Name='{clean_name}', Age='{age}'")
            ...
            try:
                ...
                # VERIFY SAVE
                with conn.cursor() as cur:
                    cur.execute("SELECT name, age, conversation_step FROM public.users WHERE phone = %s", (sender,))
                    verify = cur.fetchone()
                    logger.info(f"✅ DB VERIFIED: name='{verify.get('name')}', age='{verify.get('age')}', step='{verify.get('conversation_step')}'")
                    
            except Exception as e:
                logger.error(f"❌ DB save failed: {e}")
            
            # ===== SEND CONFIRMATION =====
            send_whatsapp_message(sender, f"Thanks {clean_name}! Looking for '{last_mood}'...")
            
            # ===== UPDATE VARIABLES & PROCESS ORIGINAL QUERY =====
            text = last_mood  # Process ORIGINAL query
            user_name = clean_name
            user_age = age
            
            # DON'T RETURN - fall through to search logic below!
        
        # =================================================================
        # NOW DO AI ANALYSIS (after handling onboarding)
        # =================================================================
        ...
        if wants_events and not wants_businesses:
            ...
            if not events:
                logger.info("⚠️ Milvus failed - trying database")
                events = smart_search(conn, 'events', ai_data, text)
            ...
            if events:
                ...
                for e in events:
                    ...
                    # Around line 1507 - replace the event_date handling with:
                    if recurring:
                        display_date = f"📅 Every {recurring}"
                    elif event_date:
                        if isinstance(event_date, date):
                            display_date = f"📅 {event_date.strftime('%Y-%m-%d')}"
                        elif isinstance(event_date, str) and event_date.lower() != 'date not specified':
                            display_date = f"📅 {event_date}"
                    else:
                        display_date = ""
                    ...
                    if e.get('instagram_link'):
                        insta_link = e.get('instagram_link')
                        if not insta_link.startswith('http'):
                            insta_link = f"https://{insta_link}"
                        caption += f"\n📸 {insta_link}"
            ...
            if businesses:
                ...
                for b in businesses:
                    ...
                    if b.get('instagram_link'):
                        insta_link = b.get('instagram_link')
                        if not insta_link.startswith('http'):
                            insta_link = f"https://{insta_link}"
                        msg += f"\n\n📸 {insta_link}"
            ...
            if events:
                found_something = True
                intro = translate_text(f"Here are some events:", user_language)
                send_whatsapp_message(sender, intro)
                ...
                for e in events:
                    ...
                    if e.get('recurring_day'):
                        ...
                        display_date = f"📅 {e.get('event_date')}"
            ...
            if businesses:
                found_something = True
                send_whatsapp_message(sender, translate_text("Found these spots:", user_language))
        ...
        try:
            ...
            send_whatsapp_message(sender, "I know amazing places! What are you looking for? 🎯")
    finally:
        if conn: 
            postgreSQL_pool.putconn(conn)
...
if __name__ == "__main__":
    print("🚀 Yara AI Starting...")
    print("✅ ONBOARDING FIXED: Name and age now save to database properly!")
    print("✅ IMAGE FIX: Images now send properly with [media_url] as list")
    print("✅ LINKS FIX: Instagram links are now clickable")
    
    if MILVUS_AVAILABLE and MILVUS_ENDPOINT:
